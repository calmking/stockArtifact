...
static Logger logger = Logger.getLogger(Strategy1.class);
	...
	public static void main(String[] args) {
		...
		try {
			 fileWriter = new FileWriter(new File(filePath+"/strategy3-"+day+".txt"), true);
		...
		try {
			...
			for (int i = 0 ; i < result.size() ; i ++ )
			{
				...
				int lowestDay = StrategyUtils.getMinVolume(dayLine, session,90);
			
				logger.info(dayLine.getId().getObj()+" "+lowestDay);
				
				String s = dayLine.getId().getObj();
				s=s.replaceAll(".stk", "");
				s=s.replaceAll("SH", "");
				s=s.replaceAll("SZ", "");
				fileWriter.write(s+" "+lowestDay+"\r\n");
			}
			fileWriter.flush();
			fileWriter.close();
			transaction.commit();
